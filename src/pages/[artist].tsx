...
import useWindowSize from "@hooks/use-window-size";
import { Track as TrackType } from '@prisma/client';
import * as Progress from "@radix-ui/react-progress";
import * as TabsPrimitive from "@radix-ui/react-tabs";
import { appRouter } from "@server/_app";
...
interface HomeProps {
  artistData: ArtistSubmission;
  trackData: any;
}

const Home: NextLayoutPage<HomeProps> = (props) => {
  const router = useRouter();
  ...
  const [artistSubmission, setArtistSubmission] = useState<ArtistSubmission | null>(artistData);
  const [tracks, setTracks] = useState<(TrackType & { artistSubmissionId: string })[] | null>(trackData);
  ...
  let track = tracks?.map(track => ({ ...track, liked: false, id: track.id.toString(), artist: track.artist ?? undefined }))
  return (
    <>
      <Head>
        <title>Stems - Music Creating Platform</title>
      ...
      {rendered && artistSubmission && !isLoading && (
        ...
        <MusicPlayerProvider id={artist as string} isArtistSubmissions tracks={track || []}>
          <>
            <Banner artistName={artistSubmission.name} />
            <Header
              id={artist as string}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Error getting artist:", await response.text());
    ...
    artistData = {
      ...
      bio: data.bio,
      ArtistSubmissionsSocials: data.ArtistSubmissionsSocials,
    };
    tracksData = data.tracksData;
  } catch (error) {
    console.error('Error fetching data:', error);
  }
  console.log("ini artist data", artistData)
  console.log("ini track data", tracksData)
  return {
    props: {
      // trpcState: ssg.dehydrate(),
      id,
